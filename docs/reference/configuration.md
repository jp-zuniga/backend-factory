...
| `SECRET_KEY` | secret string, 64–256 chars | *required* | Django's own `SECRET_KEY` — session/signing infrastructure unrelated to the JWTs below. |
...
| Field | Type | Default | What it changes |
|---|---|---|---|
| `DATABASE_URL` | `PostgresDsn` | *required* | Parsed into Django's `DATABASES["default"]` via the derived `pg_database` property. Must carry a single host, a path (database name), a username and a password, or the config fails validation (`check_database_url`). |
| `REDIS_URL` | `RedisDsn` | *required* | Host and port for Django's cache backend, via the derived `redis_cache` property. |
...
| `JWT_SECRET_KEY` | secret string, 64–256 chars | *required* | Signing/verification secret for every token this API issues. |
